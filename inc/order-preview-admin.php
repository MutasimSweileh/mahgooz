...
$sel = Sel("login", "where id=" . $data->user_id);
?>
<section class="invoice-preview-wrapper">
    <div class="row invoice-preview">
        <!-- Invoice -->
        <div class="col-xl-9 col-md-8 col-12">
            <div class="card invoice-preview-card" style="word-wrap: normal;">
                    ...
                    <div class="d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-0">
                        <div>
                            <div class="logo-wrapper ">
                                <!--  <?= $St->icon ?>
                                <h3 class="text-primary invoice-logo"><?= $St->title ?></h3> -->
                                <img src="<?= getUpUrl($_St["logo" . $clang]) ?>" alt="<?= $St->title ?>">
                            </div>
                            <p class="card-text mb-25"><i data-feather='at-sign'></i> <?= $St->email ?></p>
                            <p class="card-text mb-0"><i data-feather='phone'></i> <?= $St->mobile ?></p>
                        </div>
                ...
                <hr class="invoice-spacing my-50">
                <div class="card-body invoice-padding pt-0">
                    <div class="row invoice-spacing">
                        <div class="col-xl-8 p-0">
                            <h6 class="mb-2"><?= $_lang["user"] ?>:</h6>
                            <p class="card-text mb-25"><i data-feather='user'></i> <?= $sel->fullname ?></p>
                            <p class="card-text mb-25"><i data-feather='at-sign'></i> <?= $sel->email ?></p>
                            <p class="card-text mb-0"><i data-feather='phone'></i> <?= $sel->phone ?></p>
                            <p class="card-text mb-0 d-none"><?= $data->note ?></p>
                        </div>
                        <div class="col-xl-4 p-0 mt-xl-0 mt-2">
                            <h6 class="mb-2"><?= $_lang["Customer"] ?>:</h6>
                        ...
                        <div class="col-md-6 d-flex justify-content-end order-md-2 order-1">
                            <div class="invoice-total-wrapper">
                                ...
                                <div class="invoice-total-item">
                                    ...
                                    <p class="invoice-total-amount"><?= number_format(($_login->role != 1 ? $data->total - $data->commission : $data->total)) ?> <?= $_lang["currency"] ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-xl-3 col-md-4 col-12 invoice-actions mt-md-0 mt-2">
            <div class="card">
                <div class="card-body">
                    <a class="btn btn-primary w-100 mb-75" href="<?= getSiteUrl("order-print-admin", $data->id) ?>" target="_blank">
                        <?= $_lang["Print"] ?>
                    </a>
                    <a class="btn btn-info w-100 mb-75" data-download=".invoice-preview-card"> <?= $_lang["saveAsImage"] ?> </a>
                    <a class="btn btn-success w-100" href="<?= getSiteUrl("site-orders") ?>">
                        <?= $_lang["BacktoOrders"] ?>
                    </a>
                </div>
            </div>
