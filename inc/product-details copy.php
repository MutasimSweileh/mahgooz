<?php
$data = Selaa("products", "where id=" . $_id);
            ...
            <div class="swiper-container mySwiper">
              <div class="swiper-wrapper">
                <div class="d-flex2 swiper-slide align-items-center justify-content-center">
                  <img src="<?= getUpUrl($data["image"]) ?>" class="img-fluid product-img" alt="product image">
                </div>
                <?php
                if ($data["product_images"]) {
                  $data2 = json_decode($data["product_images"], true);
          ...
          <div class="col-12 col-md-7">
            ...
            <ul class="product-features list-unstyled">
              ...
              <? }
                  ...
                  $stock = $data["stock"];
                  $data2 = getUser("product_colors", "where active=1 and ( product_id =" . $data["id"] . ") ");
                  if ($data2) {
                    $stock = $data2[0]["stock"];
                  ?>
              <hr>
              ...
              <div class="product-color-options">
                ...
                <?php
                    foreach ($data2 as $k => $v) {
                ?>
                  <input type="radio" data-id="<?= $v["id"] ?>" data-action="product_colors" value="<?= $v["id"] ?>" <?= (!$k ? "checked" : "") ?> class="btn-check" name="product_color" id="product_color<?= $v["id"] ?>" autocomplete="off">
                  <label type="button" for="product_color<?= $v["id"] ?>" class="btn btn-outline-secondary waves-effect"><?= ucfirst($v["title" . $clang]) ?></label>
                <? } ?>
                ...
                <div id="product_sizes">
                  <?php
                    foreach ($data2 as $k => $v) {
                      $stock = $data2[0]["stock"];
                  ?>
                    <input type="radio" data-id="<?= $v["id"] ?>" data-action="product_sizes" value="<?= $v["id"] ?>" <?= (!$k ? "checked" : "") ?> class="btn-check" name="product_size" id="product_size<?= $v["id"] ?>" autocomplete="off">
                    <label type="button" for="product_size<?= $v["id"] ?>" class="btn btn-outline-secondary waves-effect"><?= ucfirst($v["title"]) ?></label>
                  <? } ?>
                </div>
              </div>
            ...
            <div class="d-flex flex-column flex-sm-row pt-1">
              <?php $sel = Sel("cart", "where product_id=" . $data["id"]); ?>
            ...
            <div class="swiper-slide">
              <a href="<?= getSiteUrl("product-details", $v["id"]) ?>">
                ...
                <div class="img-container w-50 mx-auto py-75">
                  <img src="<?= getUpUrl($v["image"]) ?>" class="img-fluid" alt="image">
                </div>
                <div class="item-meta">
                  ...
                  <p class="card-text text-primary mb-0"><?= $v["price"] ?> <?= $_lang["currency"] ?></p>
                </div>
              </a>
            </div>
          <?php } ?>
