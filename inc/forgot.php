            ...
            <div class="card-body">
                ...
                <form data-block="#login-card" id="forgot" class="auth-login-form mt-2" action="<?= getSiteUrl("reset") ?>" method="POST" novalidate="novalidate">
                    ...
                    <div class="mb-1">
                        ...
                        <input type="email" data-validmsg="<?= $_lang["valid"] ?>" data-validmsg-email="<?= $_lang["valid-email"] ?>" class="form-control" id="login-email" name="email" placeholder="john@example.com" aria-describedby="login-email" tabindex="1" autofocus="">
                    </div>

                    <button class="btn btn-primary w-100 waves-effect waves-float waves-light" tabindex="4"><?= $_lang["forgotlink"] ?></button>
                </form>
